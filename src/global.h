...
#define d_graphicsType 0;  // 0: pressure, 1: temperature, 2: humidity
...
struct measurementData
{
  ...
  uint32_t graphicsType; // determine which graph is shown  0: pressure, 1: temperature, 2: humidity
  uint32_t startCounter ;    // total counter for starts of ESP32
  uint32_t dischgCnt ;    // counter for starts of ESP32 since last charge
  ...
  uint32_t targetMeasurementIntervalSec;    // sleep time target in seconds, controls the measurement
  uint32_t lastTargetSleeptime;             // last target standard sleep time in seconds
  uint64_t lastActualSleeptimeAfterMeasUsec;         // this is the number in usec actually used to set the sleep timer after last measurement
  uint64_t lastActualSleeptimeNotMeasUsec;         // this is the number in usec actually used to set the sleep timer when no measurement

  // data for the graph that is presently used
  uint32_t graphTimeRangeHours; // complete time range of graph (84, 42, 21, 72, 36, 18 hours)
  float graphYDisplayRange;    // display range of last axis drawn in y direction (20, 40, 100, 200, 400 hPa)
  uint32_t indexFirstPointToDraw; // the index within the data arrays of the first point to draw
  ...
  uint16_t humiHistoryMax; 
  uint16_t humiHistoryMin;
  ...
  uint16_t actHumidity;      // humidity as read by the sensor. Stored in promille as integer!
  ...
  uint32_t ageOfDatapoint[noDataPoints];  // age of data point in seconds. more positive value = older
  ...
  uint16_t humiHistory[noDataPoints];  // humidity data points in the past  in %
